import img1 from "./people/img1.png";
import img2 from "./people/img2.png";
import img3 from "./people/img3.png";
import img4 from "./people/img4.png";
import img5 from "./people/img5.png";
import img6 from "./people/img6.png";
import img7 from "./people/img7.png";
import img8 from "./people/img8.png";
import img9 from "./people/img9.png";
import img10 from "./people/img10.png";
import img11 from "./people/img11.png";
import img12 from "./people/img12.png";
import img13 from "./people/img13.png";
import img14 from "./people/img14.png";
import img15 from "./people/img15.png";
import img16 from "./people/img16.png";
import img17 from "./people/img17.png";
import img18 from "./people/img18.png";
import img19 from "./people/img19.png";
import img20 from "./people/img20.png";
import img21 from "./people/img21.png";
import img22 from "./people/img22.png";
import img23 from "./people/img23.png";
import img24 from "./NEO.png";

const ALL_COMPANY =[
    {   
        "id": "c1",
        "name": "Neo X",
        "description": "Neo X is an EVM-compatible sidechain creating a bridge between Neo N3 and the widely adopted EVM network",
        "image": img24,
        "employees":[ {
            "id": "e1",
            "name": "James",
            "img": img1
        },
        {
            "id": "e2",
            "name": "John",
            "img": img2
        },
        {
            "id": "e3",
            "name": "Robert",
            "img": img3
        },
        {
            "id": "e4",
            "name": "Michael",
            "img": img4
        },
        {
            "id": "e5",
            "name": "William",
            "img": img5
        },
        {
            "id": "e6",
            "name": "William",
            "img": img6
        },
        {
            "id": "e7",
            "name": "David",
            "img": img7
        },
    ]
    },
    {   
        "id": "c2",
        "name": "GMX",
        "description": "GMX 是一个去中心化的现货和永续合约交易所，支持低交易费用和零价格影响交易。交易由独特的多资产池支持，该池通过做市、交易费和杠杆交易赚取流动性提供者费用。",
        "image": "https://public.rootdata.com/images/b18/1666566602151.jpg",
        "employees":[{
            "id": "e8",
            "name": "Richard",
            "img": img8
        },
        {
            "id": "e9",
            "name": "Joseph",
            "img": img9
        },
        {
            "id": "e10",
            "name": "Charles",
            "img": img10
        },
        {
            "id": "e11",
            "name": "Thomas",
            "img": img11
        },
        {
            "id": "e12",
            "name": "Christopher",
            "img": img12
        },
        {
            "id": "e13",
            "name": "Daniel",
            "img": img13
        }
    ]
    },
    {   
        "id": "c3",
        "name": "Uniswap",
        "description": "Uniswap是一种用于提供流动性和交易加密货币的去中心化交易协议，它消除了受信任的中介和不必要的租金提取形式，允许安全、可访问和高效的交易活动。",
        "image": "https://public.rootdata.com/images/b16/1666537174028.jpg",
        "employees":[
        {
            "id": "e14",
            "name": "Matthew",
            "img": img14
        },
        {
            "id": "e15",
            "name": "Anthony",
            "img": img15
        },
        {
            "id": "e16",
            "name": "Donald",
            "img": img16
        },
        {
            "id": "e17",
            "name": "Mark",
            "img": img17
        },
        {
            "id": "e18",
            "name": "Paul",
            "img": img18
        },
        {
            "id": "e19",
            "name": "Steven",
            "img": img19
        }
        ]
    },
    {   
        "id": "c4",
        "name": "io.net",
        "description": "io.net 是一个去中心化计算网络，支持在 Solana 区块链上开发、执行和扩展 ML 应用程序，将 100 万个 GPU 组合在一起，形成世界上最大的 GPU 集群和 DePIN.io.net 聚合来自未充分利用的资源（例如独立数据中心、加密矿工以及 Filecoin、Render 等加密项目）的 GPU 来解决这个问题，这些资源结合在去中心化物理基础设施网络 (DePIN) 中，使工程师能够在可访问、可定制、经济高效且易于实施的系统中获得大量计算能力。",
        "image": "https://public.rootdata.com/images/b6/1701757052301.jpg",
        "employees":[
            {
                "id": "e20",
                "name": "Andrew",
                "img": img20
            },
            {
                "id": "e21",
                "name": "Kenneth",
                "img": img21
            },
            {
                "id": "e22",
                "name": "Joshua",
                "img": img22
            },
            {
                "id": "e23",
                "name": "Kevin",
                "img": img23
            },
            {
                "id": "e24",
                "name": "Brian",
                "img": img1
            },
            {
                "id": "e25",
                "name": "George",
                "img": img2
            },
        ]
    },
    {   
        "id": "c5",
        "name": "Debank",
        "description": "DeBank 是一个多链 DeFi 投资组合跟踪器，支持跨多个链的 DeFi 协议。",
        "image": "https://public.rootdata.com/images/b19/1666614774471.jpg",
        "employees":[
            {
                "id": "e26",
                "name": "Edward",
                "img": img3
            },
            {
                "id": "e27",
                "name": "Ronald",
                "img": img5
            },
            {
                "id": "e28",
                "name": "Timothy",
                "img": img7
            },
            {
                "id": "e29",
                "name": "Jason",
                "img": img11
            },
            {
                "id": "e30",
                "name": "Jeffrey",
                "img": img17
            },
            {
                "id": "e31",
                "name": "Ryan",
                "img": img14
            }
        ]
    },
    {   
        "id": "c6",
        "name": "Dune",
        "description": "Dune Analytics 是一个允许任何人发布和访问由区块链数据驱动的加密仪表板的网站。 Dune 为用户提供了从区块链中查询、提取和可视化大量数据的所有工具，任何人都可以免费创建对区块链数据的 SQL 查询，结果以图表的形式可视化。",
        "image": "https://public.rootdata.com/images/b19/1666614539447.jpg",
        "employees":[
            {
                "id": "e32",
                "name": "Richard",
                "img": img8
            },
            {
                "id": "e33",
                "name": "Joseph",
                "img": img9
            },
            {
                "id": "e34",
                "name": "Charles",
                "img": img10
            },
            {   
                "id": "e35",
                "name": "Thomas",
                "img": img11
            },
            {
                "id": "e36",
                "name": "Christopher",
                "img": img12
            },
            {
                "id": "e37",
                "name": "Daniel",
                "img": img13
            }
        ]
    },
]
export function getAllCompany() {
    return ALL_COMPANY;
  }

  export function getCompanyById(id) {
    return ALL_COMPANY.find((event) => event.id === id);
  }